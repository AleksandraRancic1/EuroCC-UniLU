...
def get_n_cores() -> int:
    """Use SLURM allocation if available, otherwise fall back to host CPU count."""
    return int(os.environ.get("SLURM_CPUS_PER_TASK", multiprocessing.cpu_count()))
...
def benchmark_chunk_sizes(parquet_path: str) -> dict:
    ...
    candidate_parts = [1, n_cores // 2, n_cores, 2 * n_cores, 4 * n_cores]
    for nparts in candidate_parts:
...
def demonstrate_out_of_core(parquet_path: str):
    ...
    n_cores = get_n_cores()
    memory_limit_mb = int(file_mb * 4)

    n_workers = 8
    total_mem = int(os.environ.get("SLURM_MEM_PER_NODE", 64 * 1024)) * 1024 * 1024
    mem_per_worker_bytes = total_mem // n_workers

    print(f"\n  File size  : {file_mb:.0f} MB")
    print(f" Workers : {n_workers}")
    print(f"  Memory limit per worker: {mem_per_worker_bytes / 1e9:.0f} GB")
    ...
    with LocalCluster(
        ...
        with Client(cluster) as client:
            ...
            n_parts = max(n_workers * 2, n_cores)
            ddf = dd.read_parquet(parquet_path, engine="pyarrow")
            ddf = ddf.repartition(npartitions=n_parts)
            ddf["revenue"] = ddf["amount"] * ddf["quantity"]

            print(f"  Processing {ddf.npartitions} partitions with {n_workers} workers…")
            t0 = time.perf_counter()
...
if __name__ == "__main__":
  ...
  Optimal partitions : benchmark dependent (small files need fewer partitions)
